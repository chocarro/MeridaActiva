    ...
    try {
      ...
      if (signUpError) {
        const m: Record<string, string> = {
          "User already registered": "Este email ya está registrado. Intenta iniciar sesión o recupera tu contraseña.",
          ...
          "Signup requires a valid password": "La contraseña no es válida. Usa al menos 8 caracteres, una mayúscula y un número.",
        };
        throw new Error(m[signUpError.message] ?? signUpError.message);
      }

      // La confirmación de email está desactivada: iniciamos sesión automáticamente
      ...
      if (signInError) {
        // El registro fue bien pero el login automático falló: redirigir al login manualmente
        toast.success("¡Cuenta creada! Inicia sesión con tus datos.", { duration: 5000 });
        navigate("/login");
        return;
      }

      toast.success(`¡Bienvenida a MéridaActiva, ${form.nombre.trim()}! 🎉`, { duration: 5000 });
      navigate("/");
    } catch (err) {
